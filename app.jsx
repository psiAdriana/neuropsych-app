...
const TEST_CATALOG={
  pruebas:[
    {id:"briefa",  label:"BRIEF-A",               domain:"Funciones ejecutivas"},
    {id:"moca",    label:"MoCA",                  domain:"Screening cognitivo"},
    {id:"tmtfv",   label:"TMT + Fluidez Verbal",  domain:"Atención / FE"},
    {id:"stroop",  label:"Stroop",                 domain:"Atención / Inhibición"},
    {id:"wcst",    label:"WCST",                   domain:"Funciones ejecutivas"},
    {id:"ifs",     label:"IFS",                    domain:"Funciones ejecutivas"},
    {id:"waisiv",  label:"WAIS-IV",                domain:"Inteligencia (adultos)"},
    {id:"wiscv",   label:"WISC-V",                 domain:"Inteligencia (niños/as)"},
    {id:"ravlt",   label:"RAVLT",                  domain:"Memoria verbal"},
    {id:"tavec",   label:"TAVEC",                  domain:"Memoria verbal"},
    {id:"wms3",    label:"WMS-III",                domain:"Memoria"},
    {id:"bads",    label:"BADS",                   domain:"Funciones ejecutivas"},
    {id:"rey",     label:"Figura Compleja de Rey", domain:"Memoria / Visuoconstrucción"},
    {id:"papdi",   label:"PAPDI",                  domain:"Lenguaje / Denominación"},
    {id:"bnt",     label:"BNT",                    domain:"Lenguaje / Denominación"},
    {id:"caras",   label:"CARAS-R",                domain:"Atención"},
    {id:"atencion-wais",label:"Atención WAIS/WMS", domain:"Atención"},
    {id:"d2",      label:"d2",                     domain:"Atención / Concentración"},
    {id:"neuropsi",label:"NEUROPSI",               domain:"Screening cognitivo"},
    {id:"reloj",   label:"Test del Reloj",         domain:"Screening cognitivo"},
    {id:"hotel",   label:"Test del Hotel",         domain:"Funciones ejecutivas"},
  ],
  cuestionarios:[
    {id:"mbi",   label:"MBI",        domain:"Burnout",                    selfReport:true},
    {id:"snap",  label:"SNAP-IV",    domain:"TDAH (heteroinforme)",       selfReport:false},
    {id:"wurs",  label:"WURS",       domain:"TDAH retrospectivo",         selfReport:true},
    {id:"asrs",  label:"ASRS v1.1",  domain:"TDAH (síntomas actuales)",   selfReport:true},
    {id:"scl90", label:"SCL-90-R",   domain:"Síntomas psicopatológicos",  selfReport:true},
    {id:"srs",   label:"SRS",        domain:"TEA (heteroinforme)",        selfReport:false},
  ],
};
...
function buildAllTabs(){
  ...
  TEST_CATALOG.pruebas.forEach(t=>r.push(Object.assign({},t,{section:"pruebas"})));
  TEST_CATALOG.cuestionarios.forEach(t=>r.push(Object.assign({},t,{section:"cuestionarios"})));
...
function App(){
  ...
  function generarSesionVirtual(){
    if(!patient.name){setSessionStatus("⚠ Ingresá los datos del paciente primero.");return;}
    ...
    const sessionData={code,patientName:patient.name,patientDate:patient.date,questionnaires:qList,createdAt:new Date().toISOString(),status:"pendiente"};
    window.storage.set("session_"+code,JSON.stringify(sessionData),true)
      .then(()=>setSessionStatus("✅ Sesión creada. Compartí el código con el paciente."))
      .catch(()=>setSessionStatus("❌ Error al crear sesión."));
    setTimeout(()=>setSessionStatus(""),5000);
  }

  async function verificarRespuestas(){
    if(!sessionCode){setSessionStatus("⚠ Primero generá una sesión.");return;}
    try{
      const r=await window.storage.get("resp_"+sessionCode,true);
      if(!r){setSessionStatus("⏳ Sin respuestas todavía.");return;}
      ...
      setSessionStatus("✅ Respuestas cargadas automáticamente.");
      ...
      <aside style={S.sidebar}>
        ...
        <div style={S.sidebarLogo}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:36,height:36,borderRadius:10,background:"rgba(237,105,116,0.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>🧠</div>
            <div>
              <div style={{color:"#fff",fontSize:13,fontWeight:700,letterSpacing:"-0.01em"}}>Neuropsicología</div>
              <div style={{color:"rgba(200,216,228,0.6)",fontSize:10}}>Plataforma clínica</div>
            </div>
          </div>
        </div>
        ...
        {/* Paciente activo */}
        {patient.name&&(
          <div style={{margin:"12px 12px 4px",background:"rgba(237,105,116,0.1)",borderRadius:8,padding:"10px 12px",border:"1px solid rgba(237,105,116,0.2)"}}>
            <div style={{color:"#fff",fontSize:12,fontWeight:600,marginBottom:2}}>{patient.name}</div>
            <div style={{color:"rgba(200,216,228,0.7)",fontSize:10}}>{patient.age&&patient.age+" años · "}{patient.date}</div>
            <button onClick={saveCurrentPatient} style={{marginTop:8,width:"100%",padding:"5px 0",borderRadius:6,background:"rgba(237,105,116,0.25)",border:"1px solid rgba(237,105,116,0.4)",color:"#fff",fontSize:11,cursor:"pointer",fontWeight:600}}>
              {saveStatus||"💾 Guardar"}
            </button>
          </div>
        )}
        ...
        {/* Nav: Meta */}
        <div style={S.sidebarSection}>General</div>
        {[
        ...
        {/* Nav: Pruebas */}
        <div style={S.sidebarSection}>Pruebas cognitivas</div>
        <div style={{padding:"4px 8px 6px"}}>
          <input value={section==="pruebas"?searchFilter:""} onFocus={()=>setSection("pruebas")} onChange={e=>{setSection("pruebas");setSearchFilter(e.target.value);}} placeholder="🔍 Buscar..." style={{width:"100%",padding:"6px 10px",borderRadius:6,border:"1px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.07)",color:"#fff",fontSize:11,boxSizing:"border-box",outline:"none"}}/>
        </div>
        {TEST_CATALOG.pruebas.filter(t=>!searchFilter||section!=="pruebas"||t.label.toLowerCase().includes(searchFilter.toLowerCase())||t.domain.toLowerCase().includes(searchFilter.toLowerCase())).map(t=>(
          <button key={t.id} style={{...S.sidebarItem(tab===t.id),paddingLeft:28}} onClick={()=>{setSection("pruebas");setTab(t.id);}}>
            <span style={{width:7,height:7,borderRadius:"50%",background:adminTests[t.id]?C.soft:"rgba(240,192,200,0.2)",flexShrink:0}}/>
            <span style={{fontSize:12}}>{t.label}</span>
          </button>
        ))}

        {/* Nav: Cuestionarios */}
        <div style={S.sidebarSection}>Cuestionarios</div>
        <div style={{padding:"4px 8px 6px"}}>
          <input value={section==="cuestionarios"?searchFilter:""} onFocus={()=>setSection("cuestionarios")} onChange={e=>{setSection("cuestionarios");setSearchFilter(e.target.value);}} placeholder="🔍 Buscar..." style={{width:"100%",padding:"6px 10px",borderRadius:6,border:"1px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.07)",color:"#fff",fontSize:11,boxSizing:"border-box",outline:"none"}}/>
        </div>
        {TEST_CATALOG.cuestionarios.filter(t=>!searchFilter||section!=="cuestionarios"||t.label.toLowerCase().includes(searchFilter.toLowerCase())).map(t=>(
          <button key={t.id} style={{...S.sidebarItem(tab===t.id),paddingLeft:28}} onClick={()=>{setSection("cuestionarios");setTab(t.id);}}>
            <span style={{width:7,height:7,borderRadius:"50%",background:adminTests[t.id]?C.soft:"rgba(240,192,200,0.2)",flexShrink:0}}/>
            <span style={{fontSize:12}}>{t.label}</span>
          </button>
        ))}
      ...
      <main style={S.main}>
        ...
        <div style={S.topbar}>
          ...
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            <select value={domainFilter} onChange={e=>setDomainFilter(e.target.value)} style={{padding:"6px 10px",borderRadius:7,border:`1px solid ${C.border}`,fontSize:12,background:"#f7fafc",color:C.textMid}}>
              <option value="">Todos los dominios</option>
              {[...new Set([...TEST_CATALOG.pruebas,...TEST_CATALOG.cuestionarios].map(t=>t.domain))].sort().map(d=><option key={d} value={d}>{d}</option>)}
            </select>
        ...
        {tab==="patient"&&(
          <div>
            ...
            <div style={S.card}>
              ...
              <p style={{fontSize:12,color:C.textLight,marginBottom:14}}>Generá un código de sesión para que el paciente complete los cuestionarios desde su dispositivo. Las respuestas se cargan automáticamente en su perfil.</p>
              <div style={{marginBottom:14}}>
                <div style={{fontSize:12,fontWeight:700,color:C.primary,marginBottom:8,textTransform:"uppercase",letterSpacing:"0.05em"}}>Cuestionarios a enviar</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                  {TEST_CATALOG.cuestionarios.filter(t=>t.selfReport!==false).map(t=>(
                    <label key={t.id} style={{display:"flex",alignItems:"center",gap:6,cursor:"pointer",padding:"5px 12px",borderRadius:7,border:`1.5px solid ${selectedQForSend[t.id]?C.primary:C.border}`,background:selectedQForSend[t.id]?`${C.primary}12`:"#fff",fontSize:12}}>
              ...
              <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
                <button onClick={generarSesionVirtual} style={{padding:"8px 18px",borderRadius:8,background:C.primary,color:"#fff",border:"none",fontFamily:font,fontWeight:700,cursor:"pointer",fontSize:13}}>🔗 Generar código</button>
                {sessionCode&&(
                  <>
                    <div style={{padding:"8px 16px",borderRadius:8,background:`${C.bg}`,border:`2px dashed ${C.primary}`,fontFamily:font,fontSize:15,fontWeight:800,letterSpacing:"0.15em",color:C.primary}}>{sessionCode}</div>
                    <button onClick={()=>{navigator.clipboard.writeText(sessionCode);}} style={{padding:"8px 14px",borderRadius:8,background:"#fff",border:`1px solid ${C.border}`,fontFamily:font,fontSize:12,cursor:"pointer"}}>📋 Copiar código</button>
                    <button onClick={verificarRespuestas} style={{padding:"8px 16px",borderRadius:8,background:C.success,color:"#fff",border:"none",fontFamily:font,fontWeight:700,cursor:"pointer",fontSize:13}}>🔄 Verificar respuestas</button>
                  </>
                )}
              </div>
              {sessionCode&&<div style={{marginTop:12,padding:"10px 14px",background:`${C.bg}`,borderRadius:8,fontFamily:font,fontSize:12,color:C.textLight}}>
                Código de sesión: <strong style={{color:C.primary}}>{sessionCode}</strong> — Compartí este código con el paciente. El paciente ingresa el código en el portal de cuestionarios para completarlos online.
              </div>}
              {sessionStatus&&<div style={{marginTop:8,fontFamily:font,fontSize:12,color:sessionStatus.startsWith("✅")?C.success:sessionStatus.startsWith("⏳")?C.warning:C.danger}}>{sessionStatus}</div>}
            </div>
            <div style={S.card}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
